    ...
    var body: some View {
        ZStack {
            ...
            HStack {
                Image(systemName: "전원")
                    .font(.system(size: 18, weight: .black))
                Text(speedSimulator.start ? "정지" : "시작")
                    .font(.system(size: 18, weight: .regular))
            }
        }
    }
...
struct Data {
    static let regions = [
        Region(name: "미국", imageName: "🇺🇸", strength: 3),
        Region(name: "캐나다", imageName: "🇨🇦", strength: 2),
        Region(name: "프랑스", imageName: "🇫🇷", strength: 2)
    ]
    
    static let menus = [
        MenuItem(name: "앱은 지금 VPN 사용중", imageName: "icloud.and.arrow.down"),
        MenuItem(name: "앱평가", imageName: "star"),
        MenuItem(name: "서포트", imageName: "questionmark.circle"),
        MenuItem(name: "세팅", imageName: "gearshape"),
    ]
}
